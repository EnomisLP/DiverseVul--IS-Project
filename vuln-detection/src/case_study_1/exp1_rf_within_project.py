...
6. Score held-out functions and collect pooled OOF predictions.

Leakage controls
----------------
- A fixed 5-fold within-project StratifiedKFold manifest is reused unchanged.
- Word TF-IDF, character TF-IDF, and TruncatedSVD are fitted inside each
  training fold only.
- Project overlap between train and test is deliberate for this secondary
  within-project protocol; it must not be interpreted as unseen-project generalization.
...
EXP1_VERSION = "cs1-exp1-svd-static-rf-v2-oob-threshold-within-project"
...
def _run_single_fold(
    ...
    _log(f"{label} | scoring mixed-project held-out functions...", config.verbose)
...
def run_exp1_profile_fold(
    ...
    fold and are not the official five-fold within-project EXP-1 result.
    """
    ...
    evaluation_paths = save_evaluation_artifacts(
        ...
        additional_metadata={
            ...
            "protocol_interpretation": (
                ...
                "and expected; do not interpret this as unseen-project generalization."
            ),
            "model": (
...
def run_exp1(
    ...
    """
    Run the official five-fold within-project StratifiedKFold CS1-EXP1-RF experiment.

    Run ``run_exp1_profile_fold`` first after adding this module to Colab.
    """
    _validate_config(config)
    _log(
        f"CS1-EXP1 official run started: {config.n_splits}-fold within-project StratifiedKFold.",
        config.verbose,
    )
    _log(

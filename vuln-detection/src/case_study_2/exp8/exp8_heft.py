...
def train_heft_model(
    ...
    # 4 was a reasonable default for CodeBERTa's 6-layer stack (EXP-5) but
    # makes little sense as a default for NeoBERT's 28 layers -- it would
    # target a very early layer. The PDD's original Track-B design (written
    # for NeoBERT specifically) suggested intervening on layers {12,16,20,24}
    # (post-Pre-RMSNorm, deep-but-not-final). 16 is a reasonable single-layer
    # default within that range; the caller can override per fold/config.
    layer_target=16,
    heft_alpha=16,
    lora_lr=2e-4,
    reft_lr=2e-4,
    ...
    (pyreft-wrapped) model, matching what run_exp8_* in exp8_nested_pipeline.py
    ...
    lora_model = get_lora_model(
        model_name=DEFAULT_NEOBERT_MODEL,
        rank=rank,
    ...
    heft_model = attach_reft_to_lora_model(
        ...
        model_name=DEFAULT_NEOBERT_MODEL,  # picks the dynamic (NeoBERT) component-path resolution
    ).to(device)

...
from case_study_2.exp8.exp8_heft import train_heft_model_safe, score_model
...
EXP8_VERSION = "cs2-exp8-codeberta-heft-v1"


@dataclass(frozen=True)
class Exp8Config:
    experiment_name: str = "cs2_exp8_neobert_heft"
    ...
    # --- FIX (early stopping, applied proactively) --------------------------
    # EXP-8 is a new experiment (NeoBERT HEFT), so there's no prior fixed-epoch
    # run to point to the way EXP-5's undertraining evidence exists for
    # CodeBERTa. But EXP-7 (NeoBERT LoRA) already showed NeoBERT's 28-layer
    # stack needs meaningfully more epochs than CodeBERTa's 6 layers to
    # converge (loss still falling at epoch 4/4 under the old fixed budget),
    # and EXP-5's LoRA sub-phase was undertrained even for CodeBERTa at only
    # 2 epochs. Applying the same early-stopping approach from the start here
    # rather than waiting to discover the same problem again.
    lora_epochs: int = 10
    reft_epochs: int = 6
    ...
    reft_patience: int = 2
    reft_min_epochs: int = 1  # ReFT converges fast in EXP-5; assumed similar here, but early-stopping catches it either way
    ...
    # rank_grid matches EXP-7 (NeoBERT LoRA), not EXP-5's (8, 16) -- EXP-7's
    # search showed rank 32 winning 3/5 outer folds (see EXP-7 discussion:
    # that result is itself noisy/single-split, but there's no reason to
    # arbitrarily exclude 32 for EXP-8 when its sibling experiment kept it).
    rank_grid: Tuple[int, ...] = (8, 16, 32)
    reft_rank: int = 4
    # 4 was tuned for CodeBERTa's 6-layer stack (EXP-5); NeoBERT has 28
    # layers, so the PDD's original NeoBERT-specific suggestion (layers
    # {12,16,20,24}) is a better default. 16 is a reasonable single-layer
    # midpoint; see models.py resolve_reft_component_path for how this
    # actually gets located in NeoBERT's (different) module tree.
    layer_target: int = 16
...
def run_exp8_nested_rank(
    ...
    if device.type != "cuda":
        raise RuntimeError("EXP-8 HEFT fine-tuning requires a CUDA device.")

    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    state_path = output_dir / "exp8_nested_run_state.json"
    ...
    for outer_fold_id in fold_ids:
        ...
        if outer_fold_id == fold_ids[-1]:
            final_outer_model.save(save_directory=str(output_dir / "final_exp8_heft_adapter" / "reft"), include_model=False)
            final_outer_model.model.save_pretrained(str(output_dir / "final_exp8_heft_adapter" / "lora_adapter"))
            print(f"[nested] saved final fold HEFT adapter to {output_dir / 'final_exp8_heft_adapter'}")
    ...
    artifacts = {
        "oof_predictions": output_dir / "exp8_nested_oof_predictions.parquet",
        "selected_rank_per_fold": output_dir / "exp8_selected_rank_per_outer_fold.csv",
        "outer_training_audit": output_dir / "exp8_outer_training_audit.csv",
        "run_metadata": output_dir / "exp8_nested_run_metadata.json",
    }
    ...
    print(f"\n[nested] EXP-8 nested rank search complete in {(time.time()-t0)/60:.1f} min")
...
def run_exp8_canonical_retrain(
    development_frame: pd.DataFrame,
    tokenizer,
    selected_rank: int,
...
def run_exp8_holdout_evaluation(
    holdout_predictions: pd.DataFrame,
    config: Exp8Config,
    output_dir: Path,
    ...
    holdout_predictions.to_csv(output_dir / "exp8_holdout_predictions.csv", index=False)
    ...
    with open(output_dir / "exp8_holdout_pr_auc_bootstrap_ci.json", "w", encoding="utf-8") as f:
    ...
    plt.plot(recall, precision, color="b", label=f"EXP-8 HEFT (PR-AUC = {ap:.4f})")
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("Precision-Recall Curve - Frozen Outer Holdout")
    plt.legend(loc="lower left")
    plt.grid(True)
    plt.savefig(output_dir / "exp8_outer_holdout_pr_curve.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "exp8_outer_holdout_confusion_matrix.png")
    plt.close()
